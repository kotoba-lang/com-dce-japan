(ns dce-japan.main
  "Kotodama WASM entrypoint for the Dce_japan clean-room actor (L4) — Clojure port.
  ...
  facts (`dce_japan.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "dce_japan")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"dce_japan.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dce_japan-compat" :tier tier :entities entities} 200])
...
(defn start! [] :dce_japan-compat/ready)
